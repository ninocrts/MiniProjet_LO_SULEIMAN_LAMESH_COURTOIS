...
public class BoardFile {
	...
	private CardFIle aiCard;

	public BoardFile(BoardFile b) {
		boardCard = new CardFIle(b.getBoardCard());
	...
	public BoardFile(CardFIle inbetween, Player starter, Player off) {
		curPlayer = starter;
		offPlayer = off;
		boardCard = inbetween;
	...
	private void alphabeta() {
		...
		BoardFile temp = new BoardFile(boardCard, aiCurPlayer, aiOffPlayer);
	...
	private int alphabeta(BoardFile node, int depth, int alpha, int beta, boolean maximize) {
		if(node.checkWin()) {
		...
		if(maximize) {
			...
			for(CardFIle c : node.getCardsOfPlayerColor(aiCurPlayer.getColor())) {
				for(Piece p: node.getPiecesOfColor(aiCurPlayer.getColor())) {
					if(p == null) continue;

					for(int [] move : c.getMoves()) {
						BoardFile child = new BoardFile(node);
			...
			for(CardFIle c : node.getCardsOfPlayerColor(aiOffPlayer.getColor())) {
				if(c==null) continue;
	...
	public CardFIle[] getCardsOfPlayerColor(int color) {
	...
	public CardFIle exchange(CardFIle c) {
		CardFIle temp = boardCard;
		boardCard = c;
		return temp;
	}
	...
	public void swapCard(CardFIle handCard) {
		for(int i = 0; i < 2; i++) {
			if(curPlayer.getCards()[i].equals(handCard)) {
				curPlayer.setCard(i, boardCard);
				boardCard = new CardFIle(handCard);
				break;
			}
	...
	public BoardFile copy() {
		return new BoardFile(this);
	}
