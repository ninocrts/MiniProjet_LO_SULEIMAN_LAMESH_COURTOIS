...
public class GameGUI implements MouseListener {
	...
	private CardGUIFile blue1, blue2, red1, red2, tableCard;
	...
	public void initBar() {
		...
		menuReset.addActionListener(new MenuSelectFile());
		...
		menuPlayers.addActionListener(new MenuSelectFile());
		...
		menuDifficulty.addActionListener(new MenuSelectFile());
		...
		menuHelp.addActionListener(new MenuSelectFile());
	...
	private void initFrame(CardFIle [] list) {
...
//create card labels
		red1 = new CardGUIFile(list[1], BoardFile.red);
		red2 = new CardGUIFile(list[4], BoardFile.red);
		blue1 = new CardGUIFile(list[0], BoardFile.blue);
		blue2 = new CardGUIFile(list[3], BoardFile.blue);
		tableCard = new CardGUIFile(list[2], BoardFile.blue);
...
//add card labels mouse listener
	...
	public void reset(CardFIle [] list, BoardFile b) {
		...
		for(int y = 0; y < 5; y++)
			for(int x = 0; x < 5; x++)
				if(gameBoard.getPiece(x,y) == null) {
					...
					if(gameBoard.getPiece(x,y).getColor() == BoardFile.red)
						lblBoard[y][x].setBackground(colorRed);
					else
						lblBoard[y][x].setBackground(colorBlue);
				}
...
//Private methods for user selection/movement
	...
	private void highlightMoves(Component c) {
		...
		for(int [] pair : CardFIle.getCardByName(((CardGUIFile)lastCardHighlighted).getText()).getMoves()) {
	...
	private void updateCards() {
		CardFIle [] c = gameBoard.getPlayerOfColor(BoardFile.red).getCards();
		red1.set(new CardGUIFile(c[0], BoardFile.red));
		red2.set(new CardGUIFile(c[1], BoardFile.red));

		c = gameBoard.getPlayerOfColor(BoardFile.blue).getCards();
		blue1.set(new CardGUIFile(c[0], BoardFile.blue));
		blue2.set(new CardGUIFile(c[1], BoardFile.blue));

		tableCard.set(new CardGUIFile(gameBoard.getBoardCard(),BoardFile.blue));
	}
	...
	private void switchCard() {
		...
		CardGUIFile temp = new CardGUIFile((CardGUIFile) lastCardHighlighted);
		gameBoard.swapCard(CardFIle.getCardByName(temp.getName()));
		lastCardHighlighted = null;
		updateCards();
	}
